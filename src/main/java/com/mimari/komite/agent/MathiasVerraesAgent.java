...
@Component
public class MathiasVerraesAgent implements ExpertAgent {

    private final ChatClient chatClient;
    private final ChromaSearchService chromaSearchService;

    @Value("classpath:prompts/mathias-verraes.txt")
    private Resource promptResource;

    private String systemPrompt;

    public MathiasVerraesAgent(ChatClient.Builder builder, ChromaSearchService chromaSearchService) {
    ...
    public void init() {
        try (Reader reader = new InputStreamReader(promptResource.getInputStream(), StandardCharsets.UTF_8)) {
            ...
            throw new RuntimeException("Could not load prompt for Mathias Verraes", e);
        }
    }

    @Override
    public String getName() {
        return "Mathias Verraes (Event-Driven)";
    }

    @Override
    public String consult(String question) {
        // 1. Kitaptan ilgili bölümleri bul
        List<String> relevantDocs = chromaSearchService.search("mathias-verraes", question, 4);
