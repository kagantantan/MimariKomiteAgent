...
@Component
public class MichaelFeathersAgent implements ExpertAgent {

    private final ChatClient chatClient;
    private final ChromaSearchService chromaSearchService;

    @Value("classpath:prompts/michael-feathers.txt")
    private Resource promptResource;

    private String systemPrompt;

    public MichaelFeathersAgent(ChatClient.Builder builder, ChromaSearchService chromaSearchService) {
    ...
    public void init() {
        try (Reader reader = new InputStreamReader(promptResource.getInputStream(), StandardCharsets.UTF_8)) {
            ...
            throw new RuntimeException("Could not load prompt for Michael Feathers", e);
        }
    }

    @Override
    public String getName() {
        return "Michael Feathers (Legacy Code)";
    }

    @Override
    public String consult(String question) {
        // 1. Kitaptan ilgili bölümleri bul
        List<String> relevantDocs = chromaSearchService.search("michael-feathers", question, 4);
