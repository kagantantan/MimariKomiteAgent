...
@Component
public class SamNewmanAgent implements ExpertAgent {

    private final ChatClient chatClient;
    private final ChromaSearchService chromaSearchService;

    @Value("classpath:prompts/sam-newman.txt")
    private Resource promptResource;

    private String systemPrompt;

    public SamNewmanAgent(ChatClient.Builder builder, ChromaSearchService chromaSearchService) {
    ...
    public void init() {
        try (Reader reader = new InputStreamReader(promptResource.getInputStream(), StandardCharsets.UTF_8)) {
            ...
            throw new RuntimeException("Could not load prompt for Sam Newman", e);
        }
    }

    @Override
    public String getName() {
        return "Sam Newman (Microservices)";
    }

    @Override
    public String consult(String question) {
        // 1. Kitaptan ilgili bölümleri bul
        List<String> relevantDocs = chromaSearchService.search("sam-newman", question, 4);
