...
@Component
public class UncleBobAgent implements ExpertAgent {

    private final ChatClient chatClient;
    private final ChromaSearchService chromaSearchService;

    @Value("classpath:prompts/uncle-bob.txt")
    private Resource promptResource;

    private String systemPrompt;

    public UncleBobAgent(ChatClient.Builder builder, ChromaSearchService chromaSearchService) {
    ...
    public void init() {
        try (Reader reader = new InputStreamReader(promptResource.getInputStream(), StandardCharsets.UTF_8)) {
            ...
            throw new RuntimeException("Could not load prompt for Uncle Bob", e);
        }
    }

    @Override
    public String getName() {
        return "Uncle Bob (Clean Architecture)";
    }

    @Override
    public String consult(String question) {
        // 1. Kitaptan ilgili bölümleri bul
        List<String> relevantDocs = chromaSearchService.search("uncle-bob", question, 4);
