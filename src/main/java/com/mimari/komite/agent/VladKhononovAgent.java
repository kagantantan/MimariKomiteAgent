...
@Component
public class VladKhononovAgent implements ExpertAgent {

    private final ChatClient chatClient;
    private final ChromaSearchService chromaSearchService;

    @Value("classpath:prompts/vlad-khononov.txt")
    private Resource promptResource;

    private String systemPrompt;

    public VladKhononovAgent(ChatClient.Builder builder, ChromaSearchService chromaSearchService) {
    ...
    public void init() {
        try (Reader reader = new InputStreamReader(promptResource.getInputStream(), StandardCharsets.UTF_8)) {
            ...
            throw new RuntimeException("Could not load prompt for Vlad Khononov", e);
        }
    }

    @Override
    public String getName() {
        return "Vlad Khononov (Modern DDD)";
    }

    @Override
    public String consult(String question) {
        // 1. Kitaptan ilgili bölümleri bul
        List<String> relevantDocs = chromaSearchService.search("vlad-khononov", question, 4);
