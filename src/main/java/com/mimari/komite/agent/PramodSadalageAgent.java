...
@Component
public class PramodSadalageAgent implements ExpertAgent {

    private final ChatClient chatClient;
    private final ChromaSearchService chromaSearchService;

    @Value("classpath:prompts/pramod-sadalage.txt")
    private Resource promptResource;

    private String systemPrompt;

    public PramodSadalageAgent(ChatClient.Builder builder, ChromaSearchService chromaSearchService) {
    ...
    public void init() {
        try (Reader reader = new InputStreamReader(promptResource.getInputStream(), StandardCharsets.UTF_8)) {
            ...
            throw new RuntimeException("Could not load prompt for Pramod Sadalage", e);
        }
    }

    @Override
    public String getName() {
        return "Pramod Sadalage (Data Architecture)";
    }

    @Override
    public String consult(String question) {
        // 1. Kitaptan ilgili bölümleri bul
        List<String> relevantDocs = chromaSearchService.search("pramod-sadalage", question, 4);
