...
@Component
public class NickTuneAgent implements ExpertAgent {

    private final ChatClient chatClient;
    private final ChromaSearchService chromaSearchService;

    @Value("classpath:prompts/nick-tune.txt")
    private Resource promptResource;

    private String systemPrompt;

    public NickTuneAgent(ChatClient.Builder builder, ChromaSearchService chromaSearchService) {
    ...
    public void init() {
        try (Reader reader = new InputStreamReader(promptResource.getInputStream(), StandardCharsets.UTF_8)) {
            ...
            throw new RuntimeException("Could not load prompt for Nick Tune", e);
        }
    }

    @Override
    public String getName() {
        return "Nick Tune (Strategic DDD)";
    }

    @Override
    public String consult(String question) {
        // 1. Kitaptan ilgili bölümleri bul
        List<String> relevantDocs = chromaSearchService.search("nick-tune", question, 4);
