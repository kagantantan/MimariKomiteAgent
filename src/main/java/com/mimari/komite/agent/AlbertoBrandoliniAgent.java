...
@Component
public class AlbertoBrandoliniAgent implements ExpertAgent {

    private final ChatClient chatClient;
    private final ChromaSearchService chromaSearchService;

    @Value("classpath:prompts/alberto-brandolini.txt")
    private Resource promptResource;

    private String systemPrompt;

    public AlbertoBrandoliniAgent(ChatClient.Builder builder, ChromaSearchService chromaSearchService) {
    ...
    public void init() {
        try (Reader reader = new InputStreamReader(promptResource.getInputStream(), StandardCharsets.UTF_8)) {
            ...
            throw new RuntimeException("Could not load prompt for Alberto Brandolini", e);
        }
    }

    @Override
    public String getName() {
        return "Alberto Brandolini (Event Storming)";
    }

    @Override
    public String consult(String question) {
        // 1. Kitaptan ilgili bölümleri bul
        List<String> relevantDocs = chromaSearchService.search("alberto-brandolini", question, 4);
