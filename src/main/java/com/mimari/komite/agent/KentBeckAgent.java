...
@Component
public class KentBeckAgent implements ExpertAgent {

    private final ChatClient chatClient;
    private final ChromaSearchService chromaSearchService;

    @Value("classpath:prompts/kent-beck.txt")
    private Resource promptResource;

    private String systemPrompt;

    public KentBeckAgent(ChatClient.Builder builder, ChromaSearchService chromaSearchService) {
    ...
    public void init() {
        try (Reader reader = new InputStreamReader(promptResource.getInputStream(), StandardCharsets.UTF_8)) {
            ...
            throw new RuntimeException("Could not load prompt for Kent Beck", e);
        }
    }

    @Override
    public String getName() {
        return "Kent Beck (TDD)";
    }

    @Override
    public String consult(String question) {
        // 1. Kitaptan ilgili bölümleri bul
        List<String> relevantDocs = chromaSearchService.search("kent-beck", question, 4);
