...
@Component
public class VaughnVernonAgent implements ExpertAgent {

    private final ChatClient chatClient;
    private final ChromaSearchService chromaSearchService;

    @Value("classpath:prompts/vaughn-vernon.txt")
    private Resource promptResource;

    private String systemPrompt;

    public VaughnVernonAgent(ChatClient.Builder builder, ChromaSearchService chromaSearchService) {
    ...
    public void init() {
        try (Reader reader = new InputStreamReader(promptResource.getInputStream(), StandardCharsets.UTF_8)) {
            ...
            throw new RuntimeException("Could not load prompt for Vaughn Vernon", e);
        }
    }

    @Override
    public String getName() {
        return "Vaughn Vernon (DDD)";
    }

    @Override
    public String consult(String question) {
        // 1. Kitaptan ilgili bölümleri bul
        List<String> relevantDocs = chromaSearchService.search("vaughn-vernon", question, 4);
