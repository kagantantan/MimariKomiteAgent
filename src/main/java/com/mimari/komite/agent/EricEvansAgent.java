...
@Component
public class EricEvansAgent implements ExpertAgent {

    private final ChatClient chatClient;
    private final ChromaSearchService chromaSearchService;

    @Value("classpath:prompts/eric-evans.txt")
    private Resource promptResource;

    private String systemPrompt;

    public EricEvansAgent(ChatClient.Builder builder, ChromaSearchService chromaSearchService) {
    ...
    public void init() {
        try (Reader reader = new InputStreamReader(promptResource.getInputStream(), StandardCharsets.UTF_8)) {
            ...
            throw new RuntimeException("Could not load prompt for Eric Evans", e);
        }
    }

    @Override
    public String getName() {
        return "Eric Evans (DDD)";
    }

    @Override
    public String consult(String question) {
        // 1. Kitaptan ilgili bölümleri bul
        List<String> relevantDocs = chromaSearchService.search("eric-evans", question, 4);
