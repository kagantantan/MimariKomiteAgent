...
@Component
public class MartinFowlerAgent implements ExpertAgent {

    private final ChatClient chatClient;
    private final ChromaSearchService chromaSearchService;

    @Value("classpath:prompts/martin-fowler.txt")
    private Resource promptResource;

    private String systemPrompt;

    public MartinFowlerAgent(ChatClient.Builder builder, ChromaSearchService chromaSearchService) {
    ...
    public void init() {
        try (Reader reader = new InputStreamReader(promptResource.getInputStream(), StandardCharsets.UTF_8)) {
            ...
            throw new RuntimeException("Could not load prompt for Martin Fowler", e);
        }
    }

    @Override
    public String getName() {
        return "Martin Fowler (Refactoring & Patterns)";
    }

    @Override
    public String consult(String question) {
        // 1. Kitaptan ilgili bölümleri bul
        List<String> relevantDocs = chromaSearchService.search("martin-fowler", question, 4);
